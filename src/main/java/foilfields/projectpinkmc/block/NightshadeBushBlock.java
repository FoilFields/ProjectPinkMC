...
public class NightshadeBushBlock extends PlantBlock implements Fertilizable {
    ...
    public NightshadeBushBlock(Settings settings) {
        super(settings);
    ...
    public ItemStack getPickStack(BlockView world, BlockPos pos, BlockState state) {
        return new ItemStack(Items.NIGHTSHADE_BERRIES);
    }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        int i = state.get(AGE);
        boolean bl2 = bl = i == 3;
        if (!bl && player.getStackInHand(hand).isOf(net.minecraft.item.Items.BONE_MEAL)) {
            return ActionResult.PASS;
        }
        if (i > 1) {
            int j = 1 + world.random.nextInt(2);
            SweetBerryBushBlock.dropStack(world, pos, new ItemStack(Items.NIGHTSHADE_BERRIES, j + (bl ? 1 : 0)));
